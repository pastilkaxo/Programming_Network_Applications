...
int main(int args, _TCHAR* argv[]) {
    ...
    try {
        ...
        try {
            ...
            string obuf;
            DWORD  bytesRead = sizeof(obuf),bytesToWrite = sizeof(obuf);
            char ibuf[60];
            int msgCount;
            ...
            for (int i = 0; i < msgCount; i++) {
                ...
                Sleep(1000);
                if (!WriteFile(hPipe, obuf.c_str(), obuf.size(), &bytesToWrite, NULL)) {
                ...
                if (sscanf(ibuf, "Hello from Vlad (%d)", &receivedNumber) == 1) {
                    ...
                    bytesToWrite = obuf.size();

                    if (!WriteFile(hPipe, obuf.c_str(), bytesToWrite, &bytesToWrite, NULL)) {
                        throw SetPipeError("writefile:", GetLastError());
                    }
                }
