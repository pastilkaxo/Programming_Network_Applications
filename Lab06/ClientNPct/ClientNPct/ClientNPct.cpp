...
int main(int args, _TCHAR* argv[]) {
 ...
 /*           if ((hPipe = CreateFile(
           ...
           */
            ...
            // установка связи с каналом, выполнение одной транзакциии  и разрыв связи
            if (CallNamedPipe(TEXT("\\\\.\\pipe\\Tube"), (LPDWORD)obuf.c_str(), sizeof(obuf) - 1, ibuf, sizeof(obuf) - 1, &bytesRead, timeout)) {
                cout << "[CONNECTED TO SERVER]\n";
                }
